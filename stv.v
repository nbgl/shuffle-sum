...
Fixpoint encrypt_s_first_preference_ballot (b : s_first_preference_ballot) : ss_first_preference_ballot :=
  match b with
  | [] => []
  | (x, y, z) :: xs => (x, encrypt y, encrypt z) :: encrypt_s_first_preference_ballot xs
  end.

Fixpoint encrypt_snode_first_pref (l : list s_first_preference_ballot) : list ss_first_preference_ballot :=
  match l with
  | [] => []
  | x :: xs => encrypt_s_first_preference_ballot x :: encrypt_snode_first_pref xs
  end.
...
Proof.  intros b q s n p. induction p.
          ...
          apply sspf_ft with (t := t).
            ...
            intros i j. induction f.
              simpl. unfold sspf_ft_add_weights. simpl.
                  unfold spf_ft_add_weights in e. simpl in e.
                  rewrite -> decrypt_inverse. apply e. apply j.
              simpl. induction a.